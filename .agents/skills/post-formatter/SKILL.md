...
Ask the following questions in chat, using an interactive input control only when available:
...
Output the finished post inside a code block. No preamble. The only allowed trailing text is the next-move question in Step 5.
...
- Do not add meta-commentary about the draft. The Step 5 next-move question is allowed.
