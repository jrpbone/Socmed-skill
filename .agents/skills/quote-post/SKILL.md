...
name: quote-post
description: >
  Create a LinkedIn quote post by drafting original quote options, selecting a visual style, and generating the final image or a provider-neutral prompt. Use when the user says "quote post", "quote graphic", "motivational post", "build me a quote", or wants a low-effort LinkedIn graphic.
---
...
Build the following prompt with the quote filled in:
...
If image generation is available, inspect the reference image first, generate the new quote image directly, and return it. Otherwise output the provider-neutral prompt in a code block and tell the user to attach the reference image in their image generator.
