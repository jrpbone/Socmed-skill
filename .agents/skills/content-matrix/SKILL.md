...
Then ask the following question in chat. Use an interactive input control only when available, and accept a free-form answer:
...
Do not output the table in a fenced Markdown code block. Save it to `content-matrix-YYYY-MM-DD.md` in the current project and show the same table inline as Markdown. If the user explicitly requests an interactive view and a visualisation capability is available, create an interactive table as an additional view while keeping the Markdown file as the source of record.
...
Also offer to append the drafted post to the same `content-matrix-YYYY-MM-DD.md` file under the cell reference.
