...
description: >
  Design and generate a branded slide-by-slide LinkedIn carousel from source content, or produce provider-neutral image prompts when direct generation is unavailable. Use when the user says "carousel", "build a carousel", "turn this into a carousel", or wants multi-slide LinkedIn content. Includes an approval gate before image generation.
---
...
Wait for the content, then ask the following questions in chat. Use an interactive input control only when available:
...
Once approved, create one complete image-generation prompt per slide, numbered clearly.
...
If image generation is available, generate slides one at a time and return each result. Preserve visual consistency by including the approved brand style and preceding slide context in every generation request. Otherwise output each provider-neutral prompt in its own code block.
