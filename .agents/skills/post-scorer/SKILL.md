...
name: post-scorer
description: >
  Score a LinkedIn draft against the user's own cached, uploaded, exported, or explicitly authorised performance data. Use when the user says "score my post", "review my post", "rate this post", "give me feedback", or pastes a LinkedIn post for critique. If no performance data is available, provide a clearly labelled heuristic review instead of inventing benchmarks.
---
...
If cached data exists, use it. If not, ask the user in chat:
...
[
  {
    ...
    "multiSelect": false,
    "options": [
      {"label": "Upload or use an export", "description": "Use a LinkedIn export, CSV, JSON, or cached project data."},
      {"label": "Scrape my posts", "description": "Use a configured Apify integration after I approve the paid action."},
      {"label": "Heuristic review", "description": "Review structure and voice without claiming data-backed performance."}
    ]
  }
]
```
...
2. Confirm the actor, estimated cost, data destination, and obtain explicit approval immediately before the paid call
3. Call the configured Apify capability with `{ "username": "[their-username]", "total_posts": 100 }`
4. Download results without a fields filter, which can strip engagement data
5. Save as `[username]-all-posts.json` in the project
6. Proceed to analysis

If "Upload or use an export":
Ask for the file or locate the user-approved project file, validate its columns, and proceed only with metrics actually present.

If "Heuristic review":
Use voice-system and general editorial criteria. Mark the result `Data source: heuristic review` and do not include fabricated averages or top-decile comparisons.
...
When no performance data is available, score hook clarity, voice match, value density, mobile readability, and publish readiness. Explain that the score is editorial, not predictive. Omit `Posts analysed`, `Top 10% avg engagement`, and any percentage comparison that cannot be calculated.

## Rules

- Prefer the user's real data, but never block a clearly labelled heuristic review when the user chooses it.
- In data-backed mode, every score and fix must reference specific data points. In heuristic mode, tie each judgement to an observable property of the draft or voice files.
