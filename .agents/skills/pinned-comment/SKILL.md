---
name: pinned-comment
description: Write LinkedIn pinned comments and matching image concepts in Charlie Hills' deadpan, self-deprecating style. Use when the user asks for a pinned comment, pin comment, or first comment in this style. Produce the image concept first, then caption it with a four-line comment. Generate the image directly when available or return a provider-neutral prompt.
---
...
3. **The creator is the lower-status figure.** Always. The tool, logo, colleague, customer, or family member wins. The creator loses with quiet dignity.
...
2. **The generated image**, when image generation is available; otherwise the full provider-neutral image prompt
