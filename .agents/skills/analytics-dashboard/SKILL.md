...
Create dashboard files in the current project. Reuse the project's existing React toolchain when present. Do not install Recharts or other dependencies without approval. If no application scaffold exists, create a self-contained HTML dashboard with accessible SVG or canvas charts and no build step.
...
- Responsive layout that works on laptop and large displays
...
- Current engagement rate. Compare it with an external benchmark only when a current, credible source is available and cited; otherwise omit the benchmark comparison.
