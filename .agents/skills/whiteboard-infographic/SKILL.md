...
description: >
  Design and generate a hand-drawn whiteboard infographic from a post, newsletter, blog, research note, or raw bullets. Falls back to a provider-neutral image prompt when direct generation is unavailable. Use when the user says "whiteboard infographic", "hand-drawn graphic", "turn this into a whiteboard", or wants an AI-generated infographic for a post.
---
...
Once approved, build the full prompt below with the brief inserted into the placeholder:
...
If image generation is available, generate the infographic directly and return it. Otherwise output the provider-neutral prompt in a code block.
