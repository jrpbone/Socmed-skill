...
name: voice-builder
description: >
  Build a personalised voice profile in the current project from a short interview plus 3 to 5 writing samples. Use when the user says "build my voice", "learn my voice", "set up my content system", "onboard me", "train on my writing", or provides samples and wants future content to sound like them. Produces about-me.md and voice.md in the project root.
---
...
When this skill is triggered for voice onboarding, immediately run Step 1. The next user-facing message should contain the interview questions.
...
No preamble or summary. Interview first.

## Step 1. Run the About Me interview

Ask the questions below in chat. If the current Codex surface provides an interactive input control, it may be used; otherwise present the questions as a concise numbered list. Accept free-form answers rather than forcing the suggested options.

### Batch 1 (your very first action, no text before it)

Use the following question content and suggested choices:
...
After Batch 1 is answered, ask Batch 2 using this question content:
...
Keep it under 300 words. Every line should be useful when Codex writes future content.
...
- Do not produce a separate banned-words or negative-voice file. Absence signals live inside voice.md.
