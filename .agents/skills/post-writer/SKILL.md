...
If both files exist, ask the following questions in chat. Use an interactive input control only when available, and accept free-form answers:
...
- "Suggest topics for me": read about-me.md topic pillars and voice.md, suggest 5 specific topics with a one-line angle for each, then ask the user to pick one
...
Then present a post plan and ask the following questions:
