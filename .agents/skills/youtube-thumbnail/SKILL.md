...
description: >
  Generate a branded YouTube thumbnail or a provider-neutral generation prompt from a video title, creator reference photo, and brand colours. Use when the user says "thumbnail", "youtube thumbnail", "build me a thumbnail", or wants a video cover image before writing the script.
---
...
Then ask the following questions in chat. Use an interactive input control only when available:
...
Once approved, build this image-generation prompt:
...
If image generation is available, inspect the reference image first, generate the thumbnail directly, and return it. Otherwise output the prompt in a code block and tell the user to attach the reference photo in their image generator.

## Step 5. Offer the next move

> Want me to outline the video next? I can build the hook, middle, and CTA from the thumbnail promise.
