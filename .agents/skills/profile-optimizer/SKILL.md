...
Ask the following questions in two concise chat batches. Use an interactive input control only when available, and accept free-form answers.
...
Prepare 4 separate, self-contained image-generation briefs. If image generation is available, generate each asset after the user approves its brief. Otherwise output each provider-neutral prompt in its own code block.
...
When direct generation is unavailable, tell the user to attach their headshot for the banner and profile picture prompts. The featured tiles do not need a photo.
